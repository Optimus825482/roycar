...
/* ─── Royal Spinner: çift halka navy + gold ─── */
function RoyalSpinner({ size = "md" }: { size?: RoyalLoaderProps["size"] }) {
        ...
        <div className="flex items-center gap-1.5">
            {[0, 1, 2].map((i) => (
                <div
                    ...
                    style={{
                        animation: `royalPulse 1.2s ease-in-out ${i * 0.2}s infinite`,
                    }}
                />
            ))}
        </div>
...
/* ─── Royal Pulse Logo Effect ─── */
function RoyalPulse({ size = "md" }: { size?: RoyalLoaderProps["size"] }) {
    const s = sizeConfig[size || "md"];
    return (
...
/* ─── Skeleton Row ─── */
export function RoyalSkeleton({ className }: { className?: string }) {
    return (
        <div
            className={cn(
...
/* ─── Table Skeleton ─── */
export function RoyalTableSkeleton({ rows = 5, cols = 7 }: { rows?: number; cols?: number }) {
    return (
        <>
                ...
                <tr key={i} className="border-b border-slate-100">
                    {Array.from({ length: cols }).map((_, j) => (
                        <td key={j} className="py-4 px-4">
                            <RoyalSkeleton className={cn("h-4", j === 1 ? "w-32" : j === 0 ? "w-20" : "w-16")} />
                        </td>
                    ))}
                </tr>
            ))}
...
export function RoyalLoader({
    ...
    const loaderContent = (
        <div className={cn("flex flex-col items-center justify-center", s.gap, className)} role="status" aria-label="Yükleniyor">
            {variant === "spinner" && <RoyalSpinner size={size} />}
            {variant === "pulse" && <RoyalPulse size={size} />}
            {variant === "dots" && <RoyalDots />}
            {text && (
                <span className={cn(s.text, "text-mr-text-secondary font-medium animate-pulse")}>
                    {text}
                </span>
            )}
