...
import { RoyalLoader, RoyalSkeleton } from "@/components/shared/RoyalLoader";
...
export function ApplicationDetailModal({
    ...
    const allApplications = useMemo(() => {
        ...
        return [
            ...
            ...(app.otherApplications || []).map((oa) => ({
                ...
                departmentName: oa.department.name,
    ...
    const startEvaluation = async () => {
        ...
        setEvaluating(true);
        try {
            const res = await fetch(`/api/admin/evaluations/${applicationId}/retry`, { method: "POST" });
            const json = await res.json();
            if (json.success) {
                ...
                pollRef.current = setInterval(async () => {
                    attempts++;
                    try {
                        const pollRes = await fetch(`/api/admin/applications/${applicationId}`);
                    ...
                    <div className="sticky top-0 z-10 bg-linear-to-r from-mr-navy to-mr-navy-light px-6 py-4 flex items-center gap-4 rounded-t-2xl">
                        ...
                        {loading ? (
                            <div className="flex-1 flex items-center gap-3">
                                <RoyalSkeleton className="w-12 h-12 rounded-full bg-white/20!" />
                                <div className="space-y-2 flex-1">
                                    <RoyalSkeleton className="h-5 w-40 bg-white/20!" />
                                    <RoyalSkeleton className="h-3 w-28 bg-white/15!" />
                                </div>
                            </div>
                        ) : app ? (
                            <div className="flex-1 flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-mr-gold/30 flex items-center justify-center text-white text-lg font-bold shrink-0 ring-2 ring-mr-gold/40">
                                    {app.photoPath ? (
                                        ...
                                        <img src={app.photoPath} alt={app.fullName} width={48} height={48} className="w-full h-full rounded-full object-cover" />
                                    ) : (
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <h2 className="text-lg font-heading text-white font-bold truncate">{app.fullName}</h2>
                                        {totalApplications > 1 && (
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center py-20 gap-3">
                                <svg width="48" height="48" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-slate-400">
                                    <circle cx="24" cy="24" r="20" />
                                    <path d="M15 20h18M15 28h12" />
                                </svg>
                                <p className="text-slate-600 font-medium">Başvuru bulunamadı.</p>
                            </div>
                        ) : (
                            <div className="p-6 space-y-5">
                                ...
                                <div className="animate-fade-in-scale">
                                    <h3 className="text-sm font-bold text-mr-navy flex items-center gap-2 mb-3">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-mr-gold">
                                    ...
                                    <Card className="border-slate-200">
                                        <CardContent className="pt-4">
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1">
                                                ...
                                                <InfoRow label="Toplam Başvuru" value={String(totalApplications)} />
                                            </div>
                                        </CardContent>
                                    </Card>
                                </div>
                                ...
                                <div className="animate-slide-in-up delay-50">
                                    <h3 className="text-sm font-bold text-mr-navy flex items-center gap-2 mb-3">
                                        ...
                                        Başvuru Geçmişi
                                        {totalApplications > 1 && (
                                            <Badge className="bg-mr-navy text-white text-[10px]">{totalApplications}</Badge>
                                        )}
                                    </h3>
                                            ...
                                            <tbody className="divide-y divide-slate-100">
                                                {allApplications.map((row) => (
                                                    <tr
                                                        ...
                                                        className={`cursor-pointer transition-colors ${activeAppId === row.id
                                                            ? "bg-mr-navy/5 border-l-2 border-l-mr-gold"
                                                            : "hover:bg-slate-50"
                                                            } ${row.isPrimary ? "font-medium" : ""}`}
                                                    >
                                                        <td className="px-3 py-2.5 text-mr-navy text-xs">{formatDate(row.submittedAt)}</td>
                                                        <td className="px-3 py-2.5 text-mr-navy text-xs">{row.departmentName}</td>
                                                        ...
                                                        <td className="px-3 py-2.5 text-center">
                                                            {row.score != null ? (
                                                                <span className={`font-bold text-sm ${row.score >= 70 ? "text-emerald-600" :
                                                                    row.score >= 40 ? "text-amber-600" :
                                                                        "text-red-600"
                                                                    }`}>{row.score}</span>
                                                            ) : (
                                ...
                                {activeAppId && (
                                    <>
                                        <Separator />
                                        <div className="space-y-5 animate-fade-in-scale" key={activeAppId}>
                                            <h3 className="text-sm font-bold text-mr-navy flex items-center gap-2">
                                                ...
                                                <span className="text-xs text-mr-text-secondary font-normal">
                                                    — {allApplications.find((a) => a.id === activeAppId)?.departmentName}
                                                    {" · "}
                                                    {formatDate(allApplications.find((a) => a.id === activeAppId)?.submittedAt || "")}
                                                </span>
                                            </h3>
                                            ...
                                            {activeAppId === app.id && (
                                                <>
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-sm font-semibold text-mr-navy">Durum Değiştir</span>
                                                        <Select value={app.status} onValueChange={updateStatus} disabled={statusUpdating}>
                                                            <SelectTrigger className="w-40 h-9 text-sm font-medium" aria-label="Durumu değiştir">
                                                                <SelectValue />
                                                            </SelectTrigger>
                                                            <SelectContent>
                                            ...
                                            {activeAppId === app.id && sortedCategories.length > 0 && (
                                                <div className="space-y-3">
                                                    <h4 className="text-xs font-bold text-mr-navy flex items-center gap-2">
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-mr-gold">
                                                            <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" />
                                                            <path d="M14 2v6h6M16 13H8M16 17H8M10 9H8" />
                                                        </svg>
                                                        Aday Verileri
                                                        <Badge className="bg-mr-navy text-white text-[10px]">{app.fieldValues?.length || 0} alan</Badge>
                                                    </h4>
                                                    <div className="space-y-2">
                                                        {sortedCategories.map((cat) => (
                                                            <CollapsibleSection key={cat} title={CATEGORY_LABELS[cat] || cat}>
                                            ...
                                            {activeAppId === app.id && app.responses.length > 0 && (
                                                <div className="space-y-3">
                                                    <h4 className="text-xs font-bold text-mr-navy flex items-center gap-2">
                                                        ...
                                                        Form Yanıtları
                                                        <Badge className="bg-mr-gold text-mr-navy text-[10px]">{app.responses.length}</Badge>
                                                    </h4>
                                                    <Card className="border-slate-200">
                                                        <CardContent className="pt-4 space-y-3 divide-y divide-slate-100">
                                                            {app.responses.map((r) => (
                                                                <div key={r.id} className="pt-3 first:pt-0">
                                                                    <p className="text-xs font-semibold text-mr-navy mb-1">{r.question.questionText}</p>
                                            ...
                                            {(() => {
                                                const selEval = activeAppId === app.id
                                                    ? app.evaluation
                                                    : app.otherApplications?.find((a) => a.id === activeAppId)?.evaluation;
                                                ...
                                                return (
                                                    <Card className="border-mr-navy/15 bg-linear-to-r from-mr-navy/3 to-mr-gold/5">
                                                        <CardHeader className="pb-2">
                                                            <div className="flex items-center justify-between">
                                                                <CardTitle className="text-sm font-bold text-mr-navy flex items-center gap-2">
                                                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-mr-gold">
                                                        ...
                                                        <CardContent>
                                                            {!selEval ? (
                                                                <p className="text-sm text-mr-text-secondary py-3">Henüz AI değerlendirmesi yapılmamış.</p>
                                                            ) : selEval.status === "pending" ? (
                                                                <div className="flex items-center gap-3 py-4">
                                                                    <RoyalLoader size="sm" variant="spinner" />
                                                                    <span className="text-sm text-amber-600 font-semibold">Değerlendirme devam ediyor...</span>
                                                                </div>
                                                            ) : selEval.status === "completed" && selReport ? (
                                                                <div className="space-y-4">
                                                                    <div className="flex items-center gap-4">
                                                                        <div className={`w-16 h-16 rounded-full border-3 flex items-center justify-center text-xl font-bold ${selReport.overallScore >= 70
                                                                        ...
                                                                        <div className="flex-1 space-y-1.5">
                                                                            {selReport.recommendation && (
                                                                                <Badge className={`text-xs ${RECOMMENDATION_LABELS[selReport.recommendation]?.color || "bg-slate-600 text-white"}`}>
                                                                                    {RECOMMENDATION_LABELS[selReport.recommendation]?.label || selReport.recommendation}
                                                                                </Badge>
                                                                            )}
                                                                            {selReport.summary && (
                                                                                <p className="text-sm text-mr-text-secondary leading-relaxed">{selReport.summary}</p>
                                                                            )}
                                                                        </div>
                                                                    </div>

                                                                    {(selReport.strengths?.length > 0 || selReport.weaknesses?.length > 0) && (
                                                                            ...
                                                                            {selReport.strengths?.length > 0 && (
                                                                                <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-200">
                                                                                    <h4 className="text-xs font-bold text-emerald-800 mb-2 flex items-center gap-1">
                                                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-emerald-600"><path d="M20 6L9 17l-5-5" /></svg>
                                                                                        Güçlü Yönler
                                                                                    </h4>
                                                                            ...
                                                                            {selReport.weaknesses?.length > 0 && (
                                                                                <div className="bg-red-50 rounded-lg p-3 border border-red-200">
                                                                                    <h4 className="text-xs font-bold text-red-800 mb-2 flex items-center gap-1">
                                                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-red-600"><path d="M12 9v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                                                        Zayıf Yönler
                                                                                    </h4>
                                                                                    <ul className="space-y-1">
                                                                                        {selReport.weaknesses.map((w, i) => (
                                                                    ...
                                                                    {selReport.fitAnalysis && (
                                                                        <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                                                                            <h4 className="text-xs font-bold text-blue-800 mb-1">Pozisyon Uyumu</h4>
                                                                            <p className="text-xs text-blue-800">{selReport.fitAnalysis}</p>
                                                                        </div>
                                                                    )}
                                                                </div>
                                                            ) : (
                                                                <p className="text-sm text-red-600 font-medium py-3">Değerlendirme başarısız oldu.</p>
                                                            )}
                                                        </CardContent>
                                                    </Card>
                                                );
...
function InfoRow({ label, value }: { label: string; value: string }) {
    return (
        <div className="flex items-start justify-between gap-3 py-1">
            <span className="text-xs text-mr-text-secondary shrink-0 font-medium">{label}</span>
            <span className="text-sm text-mr-navy text-right font-medium">{value || "—"}</span>
        </div>
    );
}
